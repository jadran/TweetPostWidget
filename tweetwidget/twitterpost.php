...
{
	exit('<p style="margin: 5px;">URL is invalid</p>');
}
...
if (is_null($title))
{
	exit('<p style="margin: 5px;">No Meta Tags in post</p>');
}
...
{
	exit('<p style="margin: 5px;">Bit.Ly not generated, check API keys in config.php</p>');
}
...
else
{
	...
	exit('<p class="response" style="margin: 5px; text-align: right;">Tweet Not Posted, check API keys in config.php</p>');
}
?>
